...
test_benchmark_serving_range() {
    ...
    vllm bench serve --model $model_path --host $ip_addr --port $port --backend openai --endpoint /v1/completions\
    --dataset-name random --input-len $local_input --output-len $local_output --random-range-ratio $local_ratio --max-concurrency $local_max_concurrency\
    --num-warmups 10 --num-prompts $local_num_prompts --request-rate ${local_request_rate} --ignore-eos \
